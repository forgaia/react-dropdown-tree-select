...
test('renders label when passed in', t => {
  const wrapper = toJson(shallow(<Tag tag={{ label: 'hello' }} id="abc" />))
  t.snapshot(wrapper)
})

test('call stopPropagation and stopImmediatePropagation when pill is closed', t => {
  const onDelete = spy()
  const wrapper = mount(<Tag tag={{ label: 'hello' }} id="abc" onDelete={onDelete} />)
  const event = {
    type: 'click',
    stopPropagation: spy(),
...
test('should not call onDelete when readOnly', t => {
  const onDelete = spy()
  const wrapper = mount(<Tag tag={{ label: 'hello' }} id="abc" onDelete={onDelete} readOnly />)
  wrapper.find('.tag-remove').simulate('click', mockEvent)
...
test('should not call onDelete when disabled', t => {
  const onDelete = spy()
  const wrapper = mount(<Tag tag={{ label: 'hello' }} id="abc" onDelete={onDelete} disabled />)
  wrapper.find('.tag-remove').simulate('click', mockEvent)
  t.true(onDelete.notCalled)
})
...
test('should not cause form submit', t => {
  ...
  const wrapper = mount(
    <form onSubmit={onSubmit}>
      <Tag tag={{ label: 'hello' }} id="abc" onDelete={onDelete} />
    </form>
  )
  wrapper.find('.tag-remove').simulate('click', mockEvent)
